...
internal sealed class ActivityOutboxWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInfo("ActivityOutboxWorker started. WorkerId={WorkerId}", _workerId);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            using (ScopeContext.PushProperty("CorrelationId", correlationId))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    await RecordWorkerFailureAsync("ActivityOutboxWorker", ex, ParseCorrelationId(correlationId), stoppingToken);
