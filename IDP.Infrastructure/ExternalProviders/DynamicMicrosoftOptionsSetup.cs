...
public sealed class DynamicMicrosoftOptionsSetup : IConfigureNamedOptions<MicrosoftAccountOptions>
{
    private readonly ITenantContextAccessor _tenantAccessor;
    private readonly ISecretProtector _encryptionService;
    private readonly IServiceProvider _serviceProvider;

    public DynamicMicrosoftOptionsSetup(ITenantContextAccessor tenantAccessor,
    ...
    public void Configure(MicrosoftAccountOptions options)
    {
        ...
        options.Events.OnRedirectToAuthorizationEndpoint = async context =>
        {
            ...
            var config = await resolver.ResolveAsync(
                tenantId,
                clientId,
                ExternalProviderTypes.Microsoft);

            if (config == null)
                throw new Exception("Microsoft provider not configured.");
