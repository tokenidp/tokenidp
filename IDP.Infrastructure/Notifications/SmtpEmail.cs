...
internal sealed class SmtpEmail : IEmailNotification
{
    ...
    private static void AddAttachments(MailMessage mailMessage, Dictionary<string, string> attachments)
    {
        ...
        {
            byte[] bytesArray = File.ReadAllBytes(attachment.Value);

            MemoryStream memoryStream = new MemoryStream(bytesArray);
            Attachment attachmentItem = new(memoryStream, attachment.Key);
