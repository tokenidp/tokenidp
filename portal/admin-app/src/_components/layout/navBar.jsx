...
function NavBar({ onClick, isOpen, onNavigate, onToggleTheme, theme }) {
  ...
  return (
    <aside className={`sidebar ${isOpen ? "active" : ""}`}>
      <div className="brand">
        <img src={logo} alt="TokenIDP Logo" className="brand-logo" width={150} />
      </div>
        ...
        >
          ...
          <div className="user-info">
            <div className="user-name">
              {(user?.userData?.firstName || user?.firstName || user?.userName || '').trim()} {(user?.userData?.lastName || user?.lastName || '').trim()}
            </div>
            <div className="user-handle">@{(user?.userData?.username || user?.userName || '').trim() || 'user'}</div>
          </div>
        </div>
